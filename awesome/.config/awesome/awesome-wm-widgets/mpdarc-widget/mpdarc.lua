...
local update_graphic = function(widget, stdout, _, _, _)
    ...
    if string.len(current_song) > 35 then
        current_song = string.sub(current_song, 1, 35)
    end
    current_song = string.rep(" ", 35 - #current_song) .. current_song
    ...
    if mpdstatus == "playing" then
        ...
        mpdarc_current_song_widget.markup = '<span foreground="#88C0D0">' .. current_song .. '</span>'
    elseif mpdstatus == "paused" then
        icon.image = PAUSE_ICON_NAME
        widget.colors = { beautiful.widget_main_color }
        widget.value = tonumber(mpdpercent / 100)
        mpdarc_current_song_widget.markup = [[<span foreground="#8C868A">]] .. current_song .. [[</span>]]
    else
...
mpdarc_widget:connect_signal("mouse::enter", function() show_MPD_status() end)
mpdarc_widget:connect_signal("mouse::leave", function() naughty.destroy(notification) end)
